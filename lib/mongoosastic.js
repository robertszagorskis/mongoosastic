...
function deleteByMongoId (options, cb) {
  ...
  client.delete({
    ...
    if (err && err.status === 404) {
      if (tries <= 0) {
        schema.emit('es-removed', err, res)
        return cb(err)
      }
      options.tries = --tries
      setTimeout(() => {
...
function Mongoosastic (schema, pluginOpts) {
  ...
  function postSave (doc) {
    ...
    function onIndex (err, res) {
      if (!filter || !filter(doc)) {
        schema.emit('es-indexed', err, res)
      } else {
        schema.emit('es-filtered', err, res)
      }
    }
  ...
  schema.statics.synchronize = function synchronize (inQuery, inOpts) {
    ...
    stream.on('data', doc => {
      ...
      schema.on('es-indexed', onIndex)
      schema.on('es-filtered', onIndex)
  ...
  schema.statics.flush = function flush (inCb) {
    ...
    schema.emit('es-bulkBefore', bulkBuffer);

    esClient.bulk({
      body: bulkBuffer
    }, function(err, res) {
      if (err) {
        return schema.emit('es-bulkError', err, res);
      }
      return schema.emit('es-bulkDone', res);
    });
